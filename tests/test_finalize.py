...
from support import (A, REPO_ROOT, SOURCES, build_capture, child_peak_rss_mb,
                     finalize_with_processor, identity_processor_factory,
                     reference_mix, run, wav_samples, workdir)
...
def capture_sources(d):
    return json.loads((Path(d) / "meta.json").read_text())["capture"]["sources"]


def resampled_sources(d, sources=SOURCES):
    result = {}
    for s in sources:
        raw = np.fromfile(Path(d) / s["file"], dtype=np.dtype(s["dtype"]))
        result[s["kind"]] = A.resample_to_target(
            raw.reshape(-1, s["channels"]), s["rate"], 16000)
    return result
...
def test_output_matches_the_in_memory_result():
    for loud in (False, True):
        with workdir(f"eq-{'loud' if loud else 'quiet'}") as d:
            ...
            n = finalize_with_processor(
                d, SOURCES, identity_processor_factory, save_channels=True)
...
def test_save_channels_off_removes_the_channel_wavs():
    with workdir("nochan") as d:
        ...
        finalize_with_processor(
            d, SOURCES, identity_processor_factory, save_channels=False)
...
def test_aec_cleans_saved_microphone_before_the_final_mix():
    with workdir("aec-clean") as d:
        build_capture(d, 2, seed=13)
        resampled = resampled_sources(d)
        n = min(len(resampled["mic"]), len(resampled["system"]))
        mic = resampled["mic"][:n]
        system = resampled["system"][:n]
        cleaned = np.clip(mic.astype(np.int32) - system.astype(np.int32),
                          -32768, 32767).astype(np.int16)

        got_n = finalize_with_processor(
            d, SOURCES, lambda _sample_rate: SubtractProcessor(), save_channels=True)

        assert got_n == n
        assert np.array_equal(wav_samples(d / "mic.wav"), cleaned)
        assert np.array_equal(wav_samples(d / "system.wav"), system)
        assert np.array_equal(wav_samples(d / "audio.wav"),
                              reference_mix([cleaned, system]))
        assert not list(d.glob("*.part"))


def test_aec_failure_logs_and_falls_back_to_the_aligned_original_microphone():
    with workdir("aec-fallback") as d:
        build_capture(d, 2, seed=14)
        resampled = resampled_sources(d)
        n = min(len(resampled["mic"]), len(resampled["system"]))
        mic = resampled["mic"][:n]
        system = resampled["system"][:n]
        failure = RuntimeError("processor unavailable")
        logged = []
        original_log_problem = A._log_problem
        A._log_problem = lambda what, exc=None: logged.append((what, exc))
        try:
            got_n = finalize_with_processor(
                d, SOURCES,
                lambda _sample_rate: (_ for _ in ()).throw(failure),
                save_channels=True)
        finally:
            A._log_problem = original_log_problem

        assert got_n == n
        assert np.array_equal(wav_samples(d / "mic.wav"), mic)
        assert np.array_equal(wav_samples(d / "system.wav"), system)
        assert np.array_equal(wav_samples(d / "audio.wav"), reference_mix([mic, system]))
        assert logged == [("acoustic echo cancellation failed; using original microphone",
                           failure)]
        assert not list(d.glob("*.part"))


def test_mic_head_start_is_removed_from_both_saved_channels():
    with workdir("finalize-align") as d:
        build_capture(d, 2, seed=15,
                      started_ns={"mic": 1_000_000_000,
                                  "system": 1_250_000_000})
        sources = capture_sources(d)
        resampled = resampled_sources(d, sources)
        expected_mic = resampled["mic"][4000:]
        expected_system = resampled["system"]
        n = min(len(expected_mic), len(expected_system))

        got_n = finalize_with_processor(
            d, sources, identity_processor_factory, save_channels=True)

        mic = wav_samples(d / "mic.wav")
        system = wav_samples(d / "system.wav")
        assert got_n == n == len(mic) == len(system)
        assert np.array_equal(mic, expected_mic[:n])
        assert np.array_equal(system, expected_system[:n])


def test_progress_covers_all_weighted_phases_monotonically():
    with workdir("progress-phases") as d:
        build_capture(d, 4, seed=16)
        seen = []
        finalize_with_processor(
            d, SOURCES, identity_processor_factory, on_progress=seen.append)

        assert seen and seen[-1] == 1.0
        assert all(0.0 <= value <= 1.0 for value in seen)
        assert all(after >= before for before, after in zip(seen, seen[1:]))
        assert any(0.0 < value <= 0.70 for value in seen), "no resampling progress"
        assert any(0.70 < value <= 0.75 for value in seen), "no alignment progress"
        assert any(0.75 < value <= 0.90 for value in seen), "no AEC progress"
        assert any(0.90 < value <= 1.00 for value in seen), "no mixing progress"
...
def test_a_killed_finalize_leaves_the_capture_recoverable():
    ...
    with workdir("kill") as d:
        ...
        runner.write_text(
            ...
            "from support import finalize_with_processor, identity_processor_factory\n"
            "finalize_with_processor(d, src, identity_processor_factory,\n"
            "                        save_channels=True,\n"
    ...
    for seconds in (30, 1800):
        with workdir(f"mem-{seconds}") as d:
            ...
            peaks[seconds] = child_peak_rss_mb(
                f"finalize_with_processor({str(d)!r}, SOURCES, "
                "identity_processor_factory, save_channels=True)\n")
...
if __name__ == "__main__":
    run(["test_output_matches_the_in_memory_result",
         ...
         "test_aec_cleans_saved_microphone_before_the_final_mix",
         "test_aec_failure_logs_and_falls_back_to_the_aligned_original_microphone",
         "test_mic_head_start_is_removed_from_both_saved_channels",
         "test_progress_covers_all_weighted_phases_monotonically",
